...
**Cash Flow table columns** — `renderCashFlow()` order: Year, My Age, Spouse Age, Status, My Net/mo, Spouse/mo, Combined/mo, Home/mo, Utils/mo, BAH/mo, Debt/mo, Exp/mo, Invest/mo, Health/mo, Life Ins/mo, SBP/mo, Surplus/mo, Annual, Projected Net Worth. Health/mo and Life Ins/mo are broken out via `healthcareMonthly(y)` / `lifeInsMonthly(y)` helpers (both exported on `_cf`); the Exp/mo cell shows the residual (`expensesMonthly − health − lifeIns`) so surplus math stays identical.
